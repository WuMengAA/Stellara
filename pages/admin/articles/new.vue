...
function startAutoSave() {
  ...
  autoSaveTimer = setInterval(() => {
    ...
    const draft = { title: title.value, content: content.value, categorySlug: categorySlug.value, slug: slug.value }
...
function loadDraft() {
  ...
  const raw = localStorage.getItem('article-draft')
  if (!raw) return
  try {
    const draft = JSON.parse(raw)
    if (draft.title) title.value = draft.title
    if (draft.content) content.value = draft.content
    if (draft.categorySlug) categorySlug.value = draft.categorySlug
...
async function submit() {
  ...
  saving.value = true
  try {
    await authFetch.post('/api/articles', {
